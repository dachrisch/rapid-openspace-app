...
class TestMarketplace(TestCase):
    ...
    def test_marketplace_uri(self):
        ...
        with app.test_client() as client:
            rapidos = app.container.creation_service().create('test os', datetime.now(), timedelta(minutes=30), 2)
            response = client.get(f'/rapidos/{rapidos.id}', follow_redirects=False)
